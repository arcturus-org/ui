...
// 获取闰月天数
export function getLeapMonthDaysCount(year: number) {
...
// 农历年天数
export function getLunarYearDaysCount(year: number) {
  if (_checkYear(year)) {
...
// 农历月天数
export function getLunarYearMonthDaysCount(
  year: number,
  month: number
) {
  if (_checkYear(year)) {
    return lunarInfo[year - minYear] & (0x10000 >> month)
      ? 30
      : 29;
  }
}
...
// 农历中文代号
export function getLunarDayString(day: number) {
...
export function getJQ(
  ...
  tmp1 = new Date(
    31556925974.7 * (year - 1900) +
      sTermInfo[(month - 1) * 2] * 60000 +
      Date.UTC(1900, 0, 6, 2, 5)
  );
  tmp2 = tmp1.getUTCDate();
  if (tmp2 == day) solarTerms = solarTerm[(month - 1) * 2];
  return solarTerms;
}
...
export function solarToLunar(
  ...
  if (_checkYear(year)) {
    const baseDate = new Date(minYear, 0, 31); // 基础日期 1900 年 1 月 31 日
    const objDate = new Date(year, month - 1, day); // 目标日期
    let offset = Math.floor(
      (objDate.getTime() - baseDate.getTime()) / 86400000
    ); // 偏移天数 60 * 60 * 24 * 1000 = 86400000, 1 天的毫秒数
    let monCycle = 14;
    let temp = 0;
    let i = 0;

    for (i = minYear; i < maxYear && offset > 0; i++) {
      temp = getLunarYearDaysCount(i); // 农历 year 年的总天数
      if (offset - temp < 0) {
        break;
      } else {
        offset -= temp;
